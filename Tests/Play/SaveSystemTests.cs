...
namespace Tests.Play
{
    public class SaveSystemTests
    {
        ...
        private SerializableNpcData GenerateComplexTestData(int size = 5)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                npcs.data[i] = new EchoesNpcData(new GameObject().AddComponent<EchoesNpcComponent>());
        ...
        private SerializableNpcData GenerateSimpleTestData()
        {
            ...
            npcsData.data[0] = new EchoesNpcData(new GameObject().AddComponent<EchoesNpcComponent>())
            {
                name = nom
            };
